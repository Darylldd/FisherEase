...
class PostHarvest {
    ...
    static async getById(id) {
        const query = 'SELECT * FROM post_harvest WHERE id = ?';
    ...
    static async update(id, data) {
        const query = `
            UPDATE post_harvest
            SET license_number=?, date_registered=?, first_name=?, middle_name=?, last_name=?, address=?, contact_info=?, processing_method=?, processing_location=?, civil_status=?
            WHERE id=?
        `;
        const values = [
            data.license_number,
    ...
    static async delete(id) {
        const query = 'DELETE FROM post_harvest WHERE id = ?';
